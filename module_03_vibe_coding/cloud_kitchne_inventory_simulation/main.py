"""Cloud kitchen inventory simulation: order fulfillment, stock deduction, and restocking."""
...
# Business rule constants — change these to adjust thresholds without touching logic.
PAR_LEVEL_GRAMS = 10_000       # target stock level after a restock
LOW_STOCK_THRESHOLD_GRAMS = 1_000  # stock at or below this triggers a "running low" alert
EXPIRY_WINDOW_DAYS = 5         # ingredients expiring within this many days are flagged
DATE_FORMAT = "%Y-%m-%d"       # expected format for all expiry_date strings in seed data
...
def _build_inventory_lookup(inventory_data):
    """Return a dict mapping ingredient name to its inventory record for fast access."""
...
def check_inventory_availability(inventory_data, requirements):
    ...
    inventory_lookup = _build_inventory_lookup(inventory_data)
...
def calculate_restock_needs(inventory_data, reference_date=None):
    ...
    for item in inventory_data:
        expiry_date = datetime.strptime(item["expiry_date"], DATE_FORMAT).date()
        days_until_expiry = (expiry_date - reference_date).days
        restock_reason = None
        qty_needed_grams = 0

        # Rule 1: ingredients expiring within EXPIRY_WINDOW_DAYS are restocked to
        # PAR_LEVEL_GRAMS; this rule takes priority over the stock-level rules below.
        ...
        if 0 <= days_until_expiry <= EXPIRY_WINDOW_DAYS:
        ...
        # and request a full PAR_LEVEL_GRAMS refill.
        elif item["qty_grams"] == 0:
            restock_reason = "Out of stock"
            qty_needed_grams = PAR_LEVEL_GRAMS
        # Rule 3: if stock is at or below LOW_STOCK_THRESHOLD_GRAMS but not empty,
        # request only the grams needed to bring it back up to PAR_LEVEL_GRAMS.
        elif item["qty_grams"] <= LOW_STOCK_THRESHOLD_GRAMS:
            restock_reason = "Running low on stock"
            qty_needed_grams = PAR_LEVEL_GRAMS - item["qty_grams"]
...
def process_orders_partial(
    ...
    """Process orders with per-item partial fulfillment.

    Unlike process_orders(), this function evaluates each item in an order
    independently. Items whose ingredients are available are delivered and
    their stock is deducted; items that cannot be fulfilled are skipped.
    An order is marked:
      - 'Delivered'          if every item was fulfilled
      - 'Partially Delivered' if at least one item was fulfilled
      - 'Not Delivered'      if no items could be fulfilled
    """
    processed_orders = []
    working_inventory = deepcopy(inventory_data)
    ...
    for order in order_data:
        ...
        for item in order["items"]:
            ...
            if recipe is None:
                failed_items.append({"item": item["item"], "reason": "No matching recipe"})
                order_result["items"].append({
                    "item": item["item"],
                    "qty": item["qty"],
                    "recipe_found": False,
            ...
            requirements = calculate_ingredient_requirements(recipe, item["qty"])
            availability = check_inventory_availability(working_inventory, requirements)

            if availability["all_available"]:
                # Deduct immediately so subsequent items in the same order see
                # the reduced stock rather than competing for the same grams.
                deduct_inventory(working_inventory, requirements)
                delivered_items.append(item["item"])
                order_result["items"].append({
                    "item": item["item"],
                    "qty": item["qty"],
                    "recipe_found": True,
                ...
                missing = [
                    d["ingredient"]
                    for d in availability["details"]
                    if not d["is_available"]
                ]
                reason = "Missing or insufficient ingredients: " + ", ".join(missing)
                failed_items.append({"item": item["item"], "reason": reason})
                order_result["items"].append({
                    "item": item["item"],
                    "qty": item["qty"],
                    "recipe_found": True,
        ...
        if delivered_items and not failed_items:
            order_result["fulfilled"] = True
            order_result["partial"] = False
            order_result["reason"] = "Delivered"
            update_status_entry(status_data, order["order_id"], True, "Delivered")
        elif delivered_items and failed_items:
            failed_names = ", ".join(f["item"] for f in failed_items)
            order_result["fulfilled"] = True
            order_result["partial"] = True
            order_result["reason"] = f"Partially delivered. Not fulfilled: {failed_names}"
            update_status_entry(
                status_data, order["order_id"], True, order_result["reason"]
            )
        else:
            failed_names = ", ".join(f["item"] for f in failed_items)
            order_result["fulfilled"] = False
            order_result["partial"] = False
            order_result["reason"] = f"Not delivered: {failed_names}"
            update_status_entry(
...
def print_business_summary(processed_orders, inventory_data, restock_data):
    """Print a human-readable end-of-day summary for a kitchen manager."""
    delivered = [o for o in processed_orders if o["fulfilled"]]
    not_delivered = [o for o in processed_orders if not o["fulfilled"]]

    print("\n" + "=" * 54)
    print("   CLOUD KITCHEN — END OF DAY SUMMARY")
    print("=" * 54)

    # Order counts
    print(f"\nORDERS PROCESSED:  {len(processed_orders)}")
    ...
    print("\nFINAL INVENTORY")
    for item in inventory_data:
        print(f"  {item['ingredient']:<22} {item['qty_grams']:>7}g   expires {item['expiry_date']}")

    # Restock recommendations grouped by reason
    if restock_data:
        print("\nRESTOCK RECOMMENDATIONS")
        for item in restock_data:
            print(f"  {item['item']:<22} order {item['qty_needed_grams']:>6}g   ({item['reason']})")
    else:
